...
export function MultimodalAutoPanel({ apiKey }: { apiKey: string }) {
  const [files, setFiles] = useState<any[]>([]);
  const [fileName, setFileName] = useState('');
  const [prompt, setPrompt] = useState('Describe the main objects and scene in this image.');
  ...
  return (
    <div>
      <div className="mb-2 text-sm text-gray-500">
        Method: <code>gemini.multimodal.generateAuto(prompt, filePart)</code>
      </div>
      <div className="mb-2">
      ...
      {response && (
        <div className="mt-4 bg-gray-100 p-2 rounded">
          <div className="font-bold mb-1">Multimodal Output:</div>
